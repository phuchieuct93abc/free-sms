...
app.service('Services', function ($http) {
	...
	return {
		...
		extractCapcha : function (element) {
			...
			var remainNumberWrapper = angular.element("<span class='remainNumber'></span>")
				remainNumberWrapper.append(remainNumber.trim().replace("Số lượng bản tin miễn phí còn lại trong ngày:", "Còn lại"))
				div.append(image);
			div.append(code);
			div.append(remainNumberWrapper);
			return div;

		},
		addHistory(message, receiver, status) {
			addHistoryFn({
